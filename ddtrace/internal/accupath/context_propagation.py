...
from ddtrace.internal.accupath.path_info import PathInfo, UPSTREAM_PATHWAY_ID, generate_response_pathway_id


log = get_logger(f"accupath.context.{__name__}")
...
def extract_context(var_name, default_value_generator, extraction_cast_func, copy_to_var, headers, _response=None):
    try:
        log.debug(f"accupath - extract started for {var_name}")
        HEADER = _generate_header(var_name)
        ...
        if "None" not in copy_to_var:
            log.error(f"accupath - copying var {copy_to_var} from {var_name}")
            core.set_item(copy_to_var, generate_response_pathway_id(core.get_item(var_name)))
    except Exception as e:
        log.error("accupath - error", exc_info=True)
